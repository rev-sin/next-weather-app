	...
	return (
		...
		>
			<div className="w-4/5 mx-auto flex flex-col items-center">
				<h1 id="air-heading" className="text-3xl font-bold mb-4 text-center">
					Air Quality
				</h1>
				<form
					onSubmit={handleSubmit}
					className="flex flex-col sm:flex-row gap-4 w-full max-w-lg"
					aria-label="Search city for air pollution data"
				>
				...
				<div className="flex flex-col w-full gap-8 items-center mt-4">
					{loading && (
						<p className="text-center text-blue-600" role="status">
							Loading...
						</p>
					)}
					{error && (
						...
						<div className="w-full break-words whitespace-pre-line overflow-wrap">
							<ul className="space-y-1">
								{Object.entries(data.components).map(([key, value]) => (
									<li
										key={key}
										className="flex justify-between break-words whitespace-pre-line overflow-wrap"
									>
										<span className="font-semibold break-words whitespace-pre-line overflow-wrap">
											{key.toUpperCase()}:
										</span>
										<span className="break-words whitespace-pre-line overflow-wrap">
											{value} µg/m³
										</span>
									</li>
								))}
